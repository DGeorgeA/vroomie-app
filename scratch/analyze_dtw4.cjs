...
// Generate Correlated Noise (Speech)
let speech = [];
for (let chunk = 0; chunk < 5; chunk++) {
  ...
  for(let f=0; f<10; f++) speech.push(arr);
}
...
const stdSpeech = standardize(speech);

for (const fp of fingerprints) {
  const dist = computeDTW(stdSpeech, fp.dtw_sequence);
  console.log(`${fp.id}: Speech Dist = ${dist.toFixed(3)}`);
}
